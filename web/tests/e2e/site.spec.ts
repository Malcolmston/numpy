  ...
  for (const id of TAB_IDS) {
    ...
    for (let i = 0; i < count; i++) {
      ...
      if (/^https?:\/\//.test(href)) {
        ...
        // /numpy/api/. It is produced by the Go doc generator at deploy time,
        ...
        expect(resolved.pathname, `docs link should resolve under /numpy/`).toBe('/numpy/api/');
      } else {
...
test('docs tab renders the React API reference with a package listed', async ({ page }) => {
  ...
  // numpy is a single-package module (package numpy), so at least one package link is expected.
